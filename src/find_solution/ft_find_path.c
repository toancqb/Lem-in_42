...
/*   Updated: 2019/07/09 14:28:55 by gly              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline int	ft_explore_forward(t_global *glob, t_roomlst **room_lst,
		t_roomlst *next_room)
{
	t_roomlst *elem;
		...
		t_roomlst **room_lst)
{
	...
	while (next_room != NULL)
	{
		if (!(glob->r_status[next_room->r->i] & VISITED))
		{
			...
			{
				if ((ret = ft_explore_forward(glob, room_lst, next_room))
						<= 0)
					return (ret);
			}
		}
